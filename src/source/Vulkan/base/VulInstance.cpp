...
void VulInstance::createInstance(const VulInstanceCreateInfo &info) {
    ...
    // appInfo
    auto [major, minor, patch] = info.appVersion;
    const auto appName = info.appName.c_str();
...
VulInstanceBuilder& VulInstanceBuilder::SetAppName(const std::string& appName) {
    m_createInfo.appName = appName;
    return *this;
}

VulInstanceBuilder& VulInstanceBuilder::SetAppVersion(const Version& appVersion) {
    m_createInfo.appVersion = appVersion;
    return *this;
}
